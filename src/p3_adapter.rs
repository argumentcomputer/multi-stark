...
/// Compatibility alias: the recording expression type, under the name the
/// symbolic builder used before the constraint-IR restructure.
pub type SymbolicExpression<F> = P3Expr<F>;

/// Main-trace variable (current row) as an expression.
pub fn var<F: Field>(index: usize) -> P3Expr<F> {
    let index = u32::try_from(index).expect("column index exceeds u32");
    P3Expr(Expr::main(index))
}

/// Preprocessed-trace variable (current row) as an expression.
pub fn preprocessed_var<F: Field>(index: usize) -> P3Expr<F> {
    let index = u32::try_from(index).expect("column index exceeds u32");
    P3Expr(Expr::preprocessed(index))
}
...
impl<F: Field> AirBuilder for P3AirBuilder<F> {
    ...
    // Periodic columns are rejected by `circuit_inputs_from_air`, but the
    // Plonky3 builder trait still requires a concrete variable type.
...
/// converts into [`CircuitInputs`] by evaluating the AIR (see
/// [`circuit_inputs_from_air`]) and attaching the lookups.
pub struct LookupAir<A, F: Field> {
    pub inner_air: A,
    pub lookups: Vec<Lookup<P3Expr<F>>>,
}

impl<A, F: Field> LookupAir<A, F> {
    pub fn new(inner_air: A, lookups: Vec<Lookup<P3Expr<F>>>) -> Self {
        Self { inner_air, lookups }
    }
}

impl<F: Field, A: Air<P3AirBuilder<F>>> From<LookupAir<A, F>> for CircuitInputs<F> {
    fn from(air: LookupAir<A, F>) -> Self {
        let mut inputs = circuit_inputs_from_air(&air.inner_air);
        inputs.lookups = air
            .lookups
            .into_iter()
            .map(|lookup| Lookup {
